...
Installation
The Python package is a prerequisite for the R package. So first, follow the instructions here to install the h2o4gpu Python package (either at the system level or in a Python virtual envivonment). The easiest thing to do is to pip install either the stable or bleeding edge whl file. To ensure compatibility, the Python package version number should match the R package version number.
